...
<!-- isinya -->
        ...
        <div class="card-header bg-purple  text-center">
            <div class="row ">
                ...
                <div class="col col-lg-2 text-white'"><a href="dashboard.php" class="link-warning text-decoration-none fw-bold ms-2"><i class='bx bx-arrow-back'>  Keluar</i></a></div>
            </div>
            </div>
            <div class="card-body">
<form method="post">
<div class="row">
    ...
    <datalist id="datalist1">
        <?php if(mysqli_num_rows($dataselect)) {?>
            <?php while($row_brg= mysqli_fetch_array($dataselect)) {?>
                <option value="<?php echo $row_brg["idproduk"]?>"> <?php echo $row_brg["idproduk"]?>
            <?php $jsArray .= "nama_produk['" . $row_brg['idproduk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            $jsArray1 .= "harga_jual['" . $row_brg['idproduk'] . "'] = {harga_jual:'" . addslashes($row_brg['harga_jual']) . "'};";
            $jsArray2 .= "harga_modal['" . $row_brg['idproduk'] . "'] = {harga_modal:'" . addslashes($row_brg['harga_modal']) . "'};"; } ?>
        <?php } ?>
    </datalist>
  ...
  <div class="col-sm-8 col-md-8 col-lg-3 mb-3">
    ...
    <div class="input-group-append">
      ...
      <button type="submit" name="InputCart" class="btn btn-secondary btn-sm">Simpan</button>
    </div>
  </div>
  </div>
...
if(isset($_POST['InputCart']))
{
    ...
    if($liat>0){
      $cekbrg = mysqli_query($conn,"SELECT * FROM cart WHERE idproduk='$Input1' and invoice='$inv_c'");
      ...
      if($liatlg>0){
        ...
        $updateaja = mysqli_query($conn,"UPDATE cart SET qty='$baru', subtotal='$baru1' WHERE invoice='$inv_c' and idproduk='$Input1'");
        if($updateaja){ 
      ...
      $tambahdata = mysqli_query($conn,"INSERT INTO cart (invoice,idproduk,nama_produk,harga,harga_modal,qty,subtotal)
  ...
  $oi = $huruf . date("jnyGi") . sprintf("%02s", $urutan);
    ...
    if($bikincart){
      $tambahuser = mysqli_query($conn,"INSERT INTO cart (invoice,idproduk,nama_produk,harga,harga_modal,qty,subtotal)
...
<div class="bg-light p-3" style="border-radius:0.25rem;">
  <div class="row gy-3 align-items-center row-home">
    ...
    <form method="post">
      ...
      <div class="d-block d-sm-block d-md-none d-lg-none py-1"></div>
       ...
       class="btn btn-danger btn-sm px-3 mr-2"><i class="fa fa-trash-alt mr-1"></i>Hapus Semua</a>
        <button type="submit" name="import" class="btn btn-secondary btn-sm px-3">
    ...
    <div class="col-md-4 col-lg-6 mb-2 text-secondary text-right">
...
if(isset($_POST['import']))
{
    ...
    $UpdCart = mysqli_query($conn,"UPDATE inv SET
     ...
     $UpdLap = mysqli_query($conn, "INSERT INTO laporan (invoice,idproduk,nama_produk,harga,harga_modal,qty,subtotal)
     SELECT invoice,idproduk,nama_produk,harga,harga_modal,qty,subtotal FROM cart") or die (mysqli_connect_error());
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content border-0">
      ...
      <i class="fas fa-exclamation-triangle fa-4x text-warning mb-3"></i>
        <h3 class="mb-4">Apakah anda yakin ingi n keluar ?</h3>
        <button type="button" class="btn btn-secondary px-4 mr-2" data-dismiss="modal">Batal</button>
        <a href="logout.php" class="btn btn-secondary px-4">Keluar</a>
    </div>
  </div>
</div>
<!-- end Modal Exit -->
    ...
    <script type="text/javascript">
      <?php echo $jsArray,$jsArray1,$jsArray2; ?>
        function changeValue(idproduk) {
          document.getElementById("nama_produk").value = nama_produk[idproduk].nama_produk;
          document.getElementById("harga_jual").value = harga_jual[idproduk].harga_jual;
          document.getElementById("harga_modal").value = harga_modal[idproduk].harga_modal;
        };
        function InputSub() {
